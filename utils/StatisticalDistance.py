...
def EmpiricalStatisticalDistance(samples_0, samples_1, 
                                 hidden_dim=32, n_layers=2, activation='GELU', n_epochs=100,
                                 train_test_split=0.8,
                                 report_accuracy=True,
                                ):
    
    discriminator = BinaryClassifier(samples_0.shape[-1], 
                                     hidden_dim=hidden_dim, 
                                     n_layers=n_layers, 
                                     activation=activation)
    
    data = torch.vstack([samples_0, samples_1])
    labels = torch.vstack([torch.zeros(samples_0.shape[0], 1), torch.ones(samples_1.shape[0], 1)])
    ...
    if report_accuracy:
        ...
        with torch.no_grad():
            ...
            acc = accuracy_score(bin_pred, training_labels)
            print(f"Training set accuracy: {acc:.3f}")
            
            cont_pred = discriminator(test_data).sigmoid()
            bin_pred = (cont_pred > 0.5).int()
            acc = accuracy_score(bin_pred, test_labels)
            print(f"Test set accuracy: {acc:.3f}")
    
    
    test_preds = discriminator(test_data).sigmoid()
    return torch.abs(test_preds[test_labels.bool()].mean() - test_preds[~test_labels.bool()].mean())
